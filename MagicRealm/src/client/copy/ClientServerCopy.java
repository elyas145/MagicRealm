package client.copy;

import java.io.IOException;
import java.io.ObjectInputStream;
import java.io.ObjectOutputStream;
import java.net.Socket;
import java.net.UnknownHostException;

import client.ClientThread;
import client.ControllerMain;
import communication.ServerNetworkHandler;

public class ClientServerCopy {
	private Socket socket = null;
	private Thread thread = null;
	private ClientThreadCopy client = null;
	private ObjectInputStream streamIn = null;
	private ObjectOutputStream streamOut = null;
	private boolean connected = false;
	private ControllerMainNew controller;

	public ClientServerCopy(ControllerMainNew controller) {
		this.controller = controller;
	}

	public void connect(String ip, int port) throws UnknownHostException,
			IOException {
		System.out.println("Establishing connection. Please wait ...");
		if (!connected) {
			try {
				this.socket = new Socket(ip, port);
				System.out.println("Connected to server: "
						+ socket.getInetAddress());
				connected = true;
			} catch (UnknownHostException e) {
				System.err.println("" + "unknown host");
				// Trace.exception(e, this);
				throw e;
			} catch (IOException e) {
				System.out.println("Unexpected exception while connecting");
				// Trace.exception(e,this);
				throw e;
			}
		} else
			System.out.println("already connected to server.");

		// setup in and out stream
		try {
			streamOut = new ObjectOutputStream(socket.getOutputStream());
			streamOut.flush();
			streamIn = new ObjectInputStream(socket.getInputStream());
			if (thread == null) {
				client = new ClientThreadCopy(this, socket);
				client.setInputStream(streamIn);
				client.start();
			}
		} catch (IOException e) {
			System.out.println("Error opening Data Input Stream");
			throw e;
		}
	}

	public boolean send(ServerNetworkHandler o) {
		if (streamOut != null) {
			try {
				streamOut.reset();
				streamOut.writeObject(o);
				streamOut.flush();
			} catch (IOException e) {
				return false;
			}
			return true;
		} else {
			return false;
		}
	}

	public void handle(Object o) {
		controller.handle(o);
	}
}
